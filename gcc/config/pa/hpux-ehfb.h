...
(struct _Unwind_Context *CONTEXT, _Unwind_FrameState *FS)
{
  ...
  if (eh_debug == -1)
    {
      eh_debug_env = getenv ("EH_DEBUG");
      eh_debug = eh_debug_env ? atoi (eh_debug_env) : 0;
    }

  result = pa_fallback_frame_state (CONTEXT, FS);

  if (eh_debug)
    printf ("FALLBACK called for ra @ 0x%p ... %s\n",
	    pc_, (result == _URC_END_OF_STACK) ? "STOP" : "KEEP_ON");
  
  return (result != _URC_END_OF_STACK);
}

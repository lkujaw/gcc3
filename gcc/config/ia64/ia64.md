...
(define_expand "extv"
  [(set (match_operand:DI 0 "gr_register_operand" "")
	(sign_extract:DI (match_operand:DI 1 "gr_register_operand" "")
			 (match_operand:DI 2 "const_int_operand" "")
			 (match_operand:DI 3 "const_int_operand" "")))]
  ""
{
  /* Sanity check on the width.  */
  if (INTVAL (operands[2]) < 1 || INTVAL (operands[2]) > 64)
    FAIL;

  /* Sanity check on the starting bit.  */
  if (INTVAL (operands[3]) < 0 || INTVAL (operands[3]) > 63)
    FAIL;
})

(define_expand "extzv"
  [(set (match_operand:DI 0 "gr_register_operand" "=r")
	(zero_extract:DI (match_operand:DI 1 "gr_register_operand" "")
			 (match_operand:DI 2 "const_int_operand" "")
			 (match_operand:DI 3 "const_int_operand" "")))]
  ""
{

...
package body Ch6 is
   ...
   begin
      ...
      --  We're about to check for "aliased", but first see if we have two
      --  identifiers in a row. If so, the first is likely a misspelling of
      --  "aliased". See similar check in P_Identifier_Declarations.
      ...
      if Token = Tok_Identifier then
         ...
         begin
            ...
            if I2 and then Bad_Spelling_Of (Tok_Aliased) then
               null;
            end if;
         end;
      end if;
   ...
   begin
      ...
      --  Simple_return_statement, no expression:

      if Token = Tok_Semicolon then
         Scan; -- past ;
         Return_Node := New_Node (N_Return_Statement, Return_Sloc);

         --  Leave Expression (Return_Node) Empty.

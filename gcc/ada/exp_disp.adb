...
package body Exp_Disp is
   ...
   procedure Collect_All_Interfaces
     (T           : Entity_Id;
   ...
   begin
      ...
      if Typ /= Etype (Typ) then
         Collect_All_Interfaces (Etype (Typ), Ancestor_Ifaces);
      ...
      Collect_All_Interfaces (Typ, Typ_Ifaces);
      ...
      --  in the init proc, and we don't need to fill them here.

      if not (Is_CPP_Class (Etype (Typ)) and then Debug_Flag_QQ) then
         if Typ = Etype (Typ)
   ...
   begin
      ...
      if Is_CPP_Class (Root_Typ) and then not Debug_Flag_QQ then

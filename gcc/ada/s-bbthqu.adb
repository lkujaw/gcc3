...
package body System.BB.Threads.Queues is
   ...
   begin
      ...
      --  There is one case that we need to treat differently. When
      --  the running thread has been removed from the ready list
      --  (Running_Thread.State /= Runnable) then it means that we are
      --  executing in the context of the thread that was executing
      --  previously, and we are just waiting for a new thread to become
      --  ready to continue. Hence, we just need to update the priority
      --  (not the queue because the thread is not in the queue).
      ...
      if Thread.State = Runnable
        and then Thread.Next /= Null_Thread_Id
   ...
   begin
      ...
      pragma Assert (Thread = Running_Thread
                     and Thread = First_Thread
                     and Thread.State /= Runnable);
   ...
   begin
      --  We can only insert threads that are ready to execute

      pragma Assert (Thread.State = Runnable);

      --  Insert at the head of queue if there is no other thread with
      --  a higher priority.

      if First_Thread = Null_Thread_Id or else
        Prio > First_Thread.Active_Priority
      then

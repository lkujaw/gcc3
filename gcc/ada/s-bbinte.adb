...
package body System.BB.Interrupts is
   ...
   --  Type used to represent the stack area for each interrupt.
   --  The stack must be aligned to 8 bytes to allow double word data
   --  movements.
   ...
   Interrupt_Handlers_Table : array (Interrupt_ID) of Interrupt_Handler;
   --  Table containing the handlers attached to the different external
   --  interrupts.
   ...
   begin
      ...
      Threads.Queues.Change_Priority
        (Self_Id, Interrupts.Priority_Of_Interrupt (Interrupt));
      ...
      --  interrupt happened. Interrupt unmasking is not done here (it
      --  will be done later by the interrupt epilogue).

...
package body System.Task_Primitives.Operations is
   ...
   begin
      ...
      if Dispatching_Policy = 'F' then
         ...
         begin
            ...
            --  Get the highest fixed priority time class ID.

            Class_Info.pc_clname (1) := 'R';
            Class_Info.pc_clname (2) := 'T';
            Class_Info.pc_clname (3) := ASCII.NUL;

            Result := priocntl (P_LWPID, P_MYID, PC_GETCID,
                                Class_Info'Address);

            --  Request the fixed priority class

            Prio_Param.pc_cid := Class_Info.pc_cid;
            Prio_Param.rt_pri := Class_Info.rt_maxpri;
            Prio_Param.rt_tqsecs := Secs;
            Prio_Param.rt_tqnsecs := Nsecs;

            Result := priocntl (P_LWPID, P_MYID, PC_SETPARMS,
              Prio_Param'Address);
   ...
   begin
      ...
      if Using_Real_Time_Class then
         ...
         Param.rt_pri := Interfaces.C.short (Prio);
         Param.rt_tqsecs := Prio_Param.rt_tqsecs;
         Param.rt_tqnsecs := Prio_Param.rt_tqnsecs;

         Result := Interfaces.C.int (
           priocntl (P_LWPID, id_t (T.Common.LL.LWP), PC_SETPARMS,
             Param'Address));
   ...
   begin
      ...
      Self_ID.Common.LL.Thread := thread_t (-1);
   ...
   is
      ...
      Opts                : Interfaces.C.long := THR_DETACHED;

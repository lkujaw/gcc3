...
--  This is the UnixWare (native threads) version of this module.
...
package System.Task_Info is
   ...
   --  The UnixWare implementation of the GNU Low-Level Interface
   --  (GNULLI) implements each Ada task as a Unix International (UI)
   --  thread.  The UI thread library distributes threads across one or
   --  more LWPs (Light-Weight Process) that are members of the same
   --  process. UnixWare distributes processes and LWPs across the
   --  available CPUs on a given machine. The pragma Task_Info
   --  provides the mechanism to control the distribution of tasks to
   --  LWPs, and LWPs to processors.
